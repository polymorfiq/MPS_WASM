...
public final class ElemSection__BehaviorDescriptor extends BaseBHDescriptor {
  private static final SAbstractConcept CONCEPT = MetaAdapterFactory.getConcept(0xf0ceec7784bd4104L, 0xb53284a17dffbb8aL, 0x39e7fc40f54c3c54L, "WebAssembly.structure.ElemSection");
  ...
  /*package*/ static int byte_size_id60rZZzHEXgh(@NotNull SNode __thisNode__) {
    ...
    ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.elems$A8kX)).visitAll((elem) -> contentLen.value += (int) ByteSized__BehaviorDescriptor.byte_size_id60rZZzHEXgh.invoke(elem));

    byte[] vecLength = Bytes.u32BytesLeb128(ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.elems$A8kX)).count());
  ...
  /*package*/ static byte[] bytes_id60rZZzHEXiv(@NotNull SNode __thisNode__) {
    ...
    ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.elems$A8kX)).visitAll((code) -> {
      byte[] elemBytes = ByteSized__BehaviorDescriptor.bytes_id60rZZzHEXiv.invoke(code);
      System.arraycopy(elemBytes, 0, bytes, i.value, elemBytes.length);
      i.value += elemBytes.length;
    });

    return bytes;
  }

  /*package*/ ElemSection__BehaviorDescriptor() {
  }
